...
import { useFetchProjectDetailQuery } from "@/services/projects";
...
const ProjectDetailsLayout = ({ children }: ProjectDetailsLayoutProps) => {
	...
	const { data: project, isLoading, isError } = useFetchProjectDetailQuery({
	...
	const projectDetailsStatsData = project ? [
		...
		{
			label: "Budget",
			value: formatAmount(Number(project.data.current_budget.amount), "NGN"),
		},
		{
			label: "Total Income",
		...
		{
			label: "Total Units",
			value: String(project.data.units.length),
		},
	] : [];
