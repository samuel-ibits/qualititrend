	...
	return (
		<>
			{sidebar_context.is_sidebar_open && (
				<div className="fixed inset-0 bg-black-900/20 backdrop-blur-sm z-[60]" onClick={()=>{sidebar_context.setIsSidebarOpen(false)}} />
			)}
			<nav className={cn('max-md:z-[60] transition-default flex gap-2 flex-col justify-between w-[278px] h-screen fixed bg-[#173E62] py-5 overflow-y-auto', {'max-lg:-translate-x-[278px]':!sidebar_context.is_sidebar_open})}>
				<Link
					href='/dashboard'
					...
					<div className='flex items-center space-x-8 justify-between py-2'>
						<ul className='flex items-center'>
							{mobileLinks.slice(0, 2).map((link) => {
								return <NavigationLink is_bottom_nav key={link.name} link={link} />;
							})}
						</ul>
						<ul className='flex items-center'>
