...
const CreateProductModal = ({
    ...
    const onSubmit: SubmitHandler<any> = async (payload) => {
        setSubmitError("")
        const {name, item_code, itemType:item_type, unit_of_measurement, valuePerUnit:cost, category_id, status, stockQuantity:quantity } = watch()
        try {
            console.log({unit_of_measurement})
            await createWarehouseItem({
                //@ts-ignore
                name, item_code, item_type:item_type.value, unit_of_measurement:unit_of_measurement.id, cost: Number(cost), status:status.value, quantity:Number(quantity), category_id:1, inventory_id:1, 
            }).unwrap().then(res=>{
                setStep("success");
            }).catch(error=>{
