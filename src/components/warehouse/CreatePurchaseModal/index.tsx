...
const CreatePurchaseModal = ({
  ...
  const total_val = Number(
    getSelectedItems()?.reduce((acc, curr, index) => {
  ...
  const onSubmit: SubmitHandler<any> = async (payload) => {
    try {
      ...
      const result = await createPurchaseOrder(_output)
        .unwrap()
        .then((res) => {
          setStep("success");
        })
          ...
          <FormProvider {...methods}>
            ...
            >
              ...
              <div className="lg:col-span-2 py-4 space-y-4">
                ...
                <p>Total Value : {formatAmount(total_val || 0, "NGN")}</p>
                <p>
                  VAT :{" "}
                  {formatAmount(Number((7.5 / 100) * (total_val || 0)), "NGN")}{" "}
                  ={" "}
                  {formatAmount(
                    Number((7.5 / 100) * (total_val || 0)) + (total_val || 0),
                    "NGN",
                  )}
                </p>
              </div>
