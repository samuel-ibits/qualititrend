...
const MaterialTransferRequests = () => {
    ...
    const { data: _data, isLoading, isError, error } = useFetchMaterialTransfersQuery()
    ...
    if (isError) {
        return (
            <div>Error loading material transfers</div>
        )
    }
        ...
        <section>
            <Table
                ...
                rowComponent={(transaction, index, length) => {
                    ...
                    return (
                        <tr
                            onClick={() => { router.push(`/requests/${id}/material-transfer`) }}
