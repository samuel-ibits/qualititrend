...
const NotificationSettings = () => {
  const { data, error, isLoading, isSuccess } =
    useFetchNotificationsQuery<FetchNotificationsResponse>();
  const [updateNotification] = useUpdateNotificationMutation();

  const [notificationSettings, setNotificationSettings] =
    useState<NotificationSettingsProps>({
      email: false,
      sms: false,
      whatsapp: false,
  ...
  const toggleNotification = async (type: keyof NotificationSettingsProps) => {
    try {
      ...
      await updateNotification({
        type,
        value: updatedSettings[type] ? 1 : 0,
      }).unwrap();
    ...
    <div className="p-6">
              ...
              {["email", "sms", "whatsapp"].map((preference) => (
                <label key={preference} className="flex items-center space-x-2">
                  <input
                    ...
                    checked={
                      notificationSettings[
                        preference as keyof NotificationSettingsProps
                      ]
                    }
                    onChange={() =>
                      toggleNotification(
      ...
      <div className="space-y-2 mb-6 p-4">
        ...
        {["request_recieved", "request_approved"].map((type) => (
          <div
            key={type}
            className="flex items-center justify-between md:flex-col md:items-start md:space-y-4"
          >
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={
                  notificationSettings[type as keyof NotificationSettingsProps]
                }
                onChange={() =>
                  toggleNotification(type as keyof NotificationSettingsProps)
                }
      ...
      <div className="space-y-2 mb-6 p-4">
        ...
        {[
          {
            text: "warehouse_low",
            title: "Stock Level Notification Settings:",
          },
          {
            text: "material_delivered",
            title: "Material Delivery Notification Settings:",
          },
        ].map((type) => (
          <div
            key={type.text}
          ...
          >
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                checked={
                  notificationSettings[
                    type.text as keyof NotificationSettingsProps
                  ]
                }
                onChange={() =>
                  toggleNotification(
