...
import { CreateItemRequest, CreateItemResponse, FetchProductsResponse, FetchWarehouseProductRequest, FetchWarehouseProductResponse } from "@/types/services/warehouse";
import { CreateExpenseRequest, CreateExpenseRequestResponse, FetchExpenseRequestDetailResponse, FetchExpenseRequestResponse } from "@/types/services/warehouse/expense-requests";
import { FetchInventoriesResponse } from "@/types/services/warehouse/inventories";
import { CreateMaterialTransferResponse, FetchMaterialTransferDetailsResponse, FetchMaterialTransfersResponse } from "@/types/services/warehouse/material-transfers";
import { CreatePurchaseOrderRequest, FetchPurchaseOrderResponse, FetchWarehousePurchaseOrder } from "@/types/services/warehouse/purchase-orders";
import { FetchSuppliersResponse } from "@/types/services/warehouse/suppliers";
import { CreateSupplyRequest, CreateSupplyRequestResponse, FetchSupplyRequestDetailResponse, FetchSupplyRequestResponse } from "@/types/services/warehouse/supply-requests";
...
const getRequest = <T>(url: string, params?: T) => {
	...
	return {
		url: !params
			? url
			: url + `?${queryString({ ...cleanedParams, })}`, method: "GET",
	};
};
...
export const warehouse = createApi({
	...
	tagTypes: ["Warehouse products", "warehouse product details",
		"warehouse purchase orders", "warehouse purchase order details",
		"suppliers", "inventories",
		"material transfers", "material transfer detail",
		"expense requests", "expense request details",
		"supply requests", "supply request details",
	],
	endpoints: (builder) => ({
		CreateWarehouseItem: builder.mutation<
			...
			CreateItemRequest
		>({
			query: (credentials) => postRequest("/create-warehouse-item", credentials),
		...
		/* }), */
		fetchProjectDetail: builder.query<FetchWarehouseProductResponse, FetchWarehouseProductRequest>({
			query: (credentials) => getRequest(`/get-warehouse-item-detail/${credentials.id}`),
		...
		fetchPurchaseOrderDetails: builder.query<FetchWarehousePurchaseOrder, FetchWarehouseProductRequest>({
			query: (credentials) => getRequest(`/purchase-order/${credentials.id}`),
			providesTags: ["warehouse purchase order details"]
		}),
		createPurchaseOrder: builder.mutation<FetchPurchaseOrderResponse, CreatePurchaseOrderRequest>({
		...
		fetchMaterialTransferDetails: builder.query<FetchMaterialTransferDetailsResponse, FetchWarehouseProductRequest>({
			query: (credentials) => getRequest(`/material-transfer/${credentials.id}`),
			providesTags: ["material transfer detail"]
		}),
		createMaterialTransfer: builder.mutation<CreateMaterialTransferResponse, CreatePurchaseOrderRequest>({
			query: (credentials) => postRequest("/create-material-transfer", credentials),
		...
		fetchExpenseRequestDetails: builder.query<FetchExpenseRequestDetailResponse, FetchWarehouseProductRequest>({
			query: (credentials) => getRequest(`/expense-request/${credentials.id}`),
			providesTags: ["expense request details"]
		}),
		createExpenseRequest: builder.mutation<CreateExpenseRequestResponse, CreateExpenseRequest>({
			query: (credentials) => postRequest("/create-expense-request", credentials),
		...
		fetchSupplyRequestDetails: builder.query<FetchSupplyRequestDetailResponse, FetchWarehouseProductRequest>({
			query: (credentials) => getRequest(`/supply-request/${credentials.id}`),
			providesTags: ["supply request details"]
		}),
		createSupplyRequest: builder.mutation<CreateSupplyRequestResponse, CreateSupplyRequest>({
			query: (credentials) => postRequest("/create-supply-request", credentials),
