...
export const projectStatus = createApi({
    ...
    endpoints: (builder) => ({
        createProjectStatus: builder.mutation<CreateProjectStatusResponse, CreateProjectStatusRequest>({
            query: (data) => postRequest("/create-project-status", data),
            invalidatesTags: ["ProjectStatus"],
        }),

        fetchProjectStatuss: builder.query<FetchProjectStatussResponse, FetchProjectStatussRequest>({
            query: (params) => getRequest("/fetch-project-status", params),
            providesTags: ["ProjectStatus"],
        }),

        updateProjectStatus: builder.mutation<UpdateProjectStatusResponse, UpdateProjectStatusRequest>({
            query: ({ id, ...data }) => putRequest(`/update-project-status/${id}`, data),
            invalidatesTags: ["ProjectStatus", "ProjectStatusDetails"],
        }),

        deleteProjectStatus: builder.mutation<DeleteProjectStatusResponse, DeleteProjectStatusRequest>({
